...
struct Camera{
  public:
    ...
    float exposure_min = 0.000003; // minimum exposure time in seconds (note: do not set to >=0, otherwise FLIR camera switched to auto-expose)
    float exposure_max = 0.002047; // maximum exposure time in seconds

    float F = 2; // camera F-number
...
class  ExpGainController{
  ...
  ExposureParameters update_params(int score=GRADINFO_SCORE,int correction=GAMMA_CORRECTIONS,int blur_estim=IMU_ESTIM);
    ...
    float gain_dB_scale = log2(5.0)/20.0 ; // Precompute scale applied to gain (in dB) to compute exposure value. It is very close to 1/6
